...
enum CommandPhraseDefaults {
    ...
    static var smallTalk: [CommandDefinition] { [
        ...
        def("farewell", "Farewell", .smallTalk,
            "Acknowledge the user's goodbye or closing phrase.", .low, [
            ("goodbye",                         .exact),
            ("bye",                             .exact),
            ("see you",                         .exact),
            ("see ya",                          .exact),
            ...
            ("night",                           .exact),
            ("bye jarvis",                      .exact),
            ("goodbye jarvis",                  .exact),
            ("good night jarvis",               .exact),
        ]),

        def("thank_user", "Thank You", .smallTalk,
            "Acknowledge the user's thanks.", .low, [
            ("thanks",                          .exact),
            ("thank you",                       .exact),
            ("thank you very much",             .exact),
            ("thanks a lot",                    .exact),
            ("cheers",                          .exact),
            ("much appreciated",                .exact),
            ("thanks jarvis",                   .exact),
            ("thank you jarvis",                .exact),
        ]),
    ] }
    ...
    static var github: [CommandDefinition] { [
        ...
        def("open_github_issue_overlay", "Open GitHub Issue Overlay", .github,
            "Opens the reviewable GitHub Issue Overlay — never silently creates.", .high, [
            ("open a github issue",                 .exact),
            ("raise a github issue",                .exact),
            ("open a github issue",                 .contains),
            ("raise a github issue",                .contains),
            ("make a bug report",                   .contains),
            ("file a bug",                          .contains),
        ]),
        def("log_as_github_issue", "Log as GitHub Issue", .github,
            "Opens GitHub Issue Overlay pre-filled with recent error context.", .high, [
            ("log this as an issue",                .exact),
            ("create an issue for this",            .exact),
            ("log this as a bug",                   .exact),
            ("log this as an issue",                .contains),
            ("create an issue for this",            .contains),
            ("report this bug",                     .contains),
            ("log it as an issue",                  .contains),
        ]),
        def("why_are_you_crashing", "Why Are You Crashing", .github,
            "Speaks the current top CPU suspect and opens diagnostics overlay.", .high, [
            ("why are you crashing",                .exact),
            ("why is your cpu high",                .exact),
            ("why are you using so much cpu",       .exact),
            ("what is making you slow",             .exact),
            ("what's causing the cpu usage",        .contains),
            ("what's using all the cpu",            .contains),
            ("why are you slow",                    .contains),
        ]),
        def("open_crash_log", "Open Crash Log", .github,
            "Opens the crash breadcrumb log file.", .medium, [
            ("open crash log",                      .exact),
            ("show crash log",                      .exact),
            ("open the log file",                   .exact),
            ("show the log",                        .contains),
            ("open diagnostic log",                 .contains),
        ]),
        def("run_stability_report", "Run Stability Report", .github,
            "Opens stability/crash diagnostic overlay pre-filled with current metrics.", .high, [
            ("run stability report",                .exact),
            ("open crash report",                   .exact),
            ("create stability issue",              .exact),
            ("stability report",                    .contains),
            ("crash report",                        .contains),
            ("performance report",                  .contains),
            ("memory report",                       .contains),
            ("diagnose jarvis",                     .contains),
        ]),
        def("enter_safe_mode", "Enter Safe Mode", .github,
            "Enters safe degraded mode to reduce CPU and memory usage.", .high, [
            ("enter safe mode",                     .exact),
            ("safe mode",                           .exact),
            ("reduce memory",                       .exact),
            ("stop everything",                     .exact),
        ]),
    ...
    static var surveillance: [CommandDefinition] { [
        ...
        def("describe_visual_scene", "Describe the Scene", .camera,
            "Motion-tracking scene description across all surveillance feeds.", .high, [
            ("scene description",               .exact),
            ("surveillance scene",              .exact),
        ]),
