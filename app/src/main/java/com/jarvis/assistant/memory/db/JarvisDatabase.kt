...
abstract class JarvisDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_19_20 = object : Migration(19, 20) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // Drop speaker enrolment tables — speaker recognition removed.
                // The tables may not exist on installs that skipped earlier
                // migrations, so IF EXISTS guards are required.
                db.execSQL("DROP TABLE IF EXISTS speaker_embeddings")
                db.execSQL("DROP TABLE IF EXISTS person_records")
                db.execSQL("DROP TABLE IF EXISTS recent_guests")
            }
        }
