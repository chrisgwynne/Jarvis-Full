...
import com.jarvis.assistant.owner.OwnerProfileStore
...
class JarvisRuntime(
    ...
    companion object {
        ...
        // How many consecutive silence timeouts to re-open the mic before
        // returning to wake-word.  Set high (20) for effectively persistent
        // listening — 20 × ~8s window = ~2.7 minutes of idle mic before
        // giving up.  A 1.5 s gap between retries gives the audio-focus
        // system time to settle so media apps (Spotify) aren't ducked
        // repeatedly by rapid recognizer restarts.
        private const val MAX_SILENCE_RETRIES    = 20
        private const val SILENCE_RETRY_DELAY_MS = 1_500L
    ...
    // ── Owner identity ─────────────────────────────────────────────────────────
    private lateinit var ownerProfileStore: OwnerProfileStore
    /** True when the owner has completed first-run name setup. */
    @Volatile private var setupComplete = false
    /** True during first-run onboarding: we've asked for the owner's name. */
    private var awaitingOwnerName = false
    ...
    fun initialize() {
        ...
        ownerProfileStore = OwnerProfileStore(context)
    ...
    fun start() {
        ...
        setupComplete = ownerProfileStore.setupComplete
    ...
    private fun startListenerWatchdog() {
        scope.launch {
            ...
            // Raised from 45 s → 180 s: the persistent-listener now legitimately
            // sits in Listening state for up to ~2.7 min between silence retries.
            // 45 s caused the watchdog to kill healthy listening sessions and was
            // the root cause of "stops after ~1 minute".
            val LISTENING_STUCK_MS        = 180_000L
    ...
    private fun onWakeWordDetected() {
        ...
        pipelineJob = scope.launch {
            ...
            try {
                ...
                // First-run onboarding: no owner name saved yet — prompt for setup.
                // Re-read from store to avoid a race with the async load in start().
                if (!setupComplete) {
                    setupComplete = ownerProfileStore.setupComplete
                }
                if (!setupComplete) {
                    speakAndRecord("Hi! I'm Jarvis. What's your name?")
                    awaitingOwnerName = true
                ...
                if (!awaitingOwnerName) {
                    ...
                    // Audio-routing settle: hardware needs 100–300ms to switch
                    // from speaker-output back to mic-input after the wake ack.
                    // Without this, speechCapture.listen() opens while the codec
                    // is still in playback mode → ERROR_AUDIO or silent results.
                    delay(250L)
                }
                ...
                while (true) {
                    ...
                    if (transcript.isBlank()) {
                        if (elapsed < FAST_FAIL_THRESHOLD) {
                            ...
                            // Natural silence timeout.  Retry up to MAX_SILENCE_RETRIES
                            // times before returning to wake-word, so the user doesn't
                            // need to say "Jarvis" again after a brief pause.  A short
                            // gap lets the audio-focus system settle between retries and
                            // avoids hammering media apps (Spotify) with rapid re-opens.
                            consecutiveFastFails = 0
                            consecutiveSilences++
                            if (consecutiveSilences <= MAX_SILENCE_RETRIES) {
                                Log.d(TAG, "[SILENCE_RETRY] attempt=$consecutiveSilences " +
                                    "max=$MAX_SILENCE_RETRIES elapsed_ms=$elapsed — re-opening mic")
                                delay(SILENCE_RETRY_DELAY_MS)
                                machine.transition(JarvisState.Listening)
                                syncState(JarvisState.Listening)
                                continue
                            }
                            // All retries exhausted — give up and return to wake-word.
                            consecutiveSilences = 0
                            Log.d(TAG, "[CONVERSATION_SILENCE_TIMEOUT] " +
                                "elapsed=${elapsed}ms — returning to wake-word after $MAX_SILENCE_RETRIES retries")
                    ...
                    //
                    // BYPASS: owner-name onboarding — Jarvis explicitly asked "What's
                    // your name?" and the reply MUST reach the handler below regardless
                    // of score.  Bare names ("Chris") score 0.0 at the attention gate
                    // and would be dropped as IGNORE, breaking first-run setup.
                    val inSpeakerFlow = awaitingOwnerName
                    if (inSpeakerFlow) {
                        Log.d(TAG, "[ATTENTION_GATE_BYPASSED] owner_name_setup")
                    }
                    if (!inSpeakerFlow) run {
                        ...
                        when (val decision = attentionGate.gate(signals)) {
                            ...
                            is AttentionDecision.AskIfForMe -> {
                                ...
                                audioFocus.abandonFocus()
                                delay(250L) // audio-routing settle before next listen()
                                machine.transition(JarvisState.Listening)
                                syncState(JarvisState.Listening)
                                continue
                            }
                    ...
                    // ── Owner name capture (first-run onboarding) ─────────────
                    if (awaitingOwnerName) {
                        val name = parseOwnerName(transcript)
                        if (name != null) {
                            ownerProfileStore.saveOwner(name)
                            setupComplete     = true
                            awaitingOwnerName = false
                            promptAssembler.invalidateProfileCache()
                            speakAndRecord("Pleased to meet you, $name.")
                            attentionGate.extendActiveWindow()
                        } else {
    ...
    private suspend fun streamAndSpeak(transcript: String, isOnline: Boolean) {
        ...
        try {
            ...
            if (settings.voiceResponse && speakingStarted) {
                ...
                // Audio-routing settle: same reason as speakAndRecord() — the
                // codec needs time to switch from speaker-out back to mic-in
                // before the next speechCapture.listen() call.
                delay(250L)
            }
    ...
    private suspend fun speakAndRecord(text: String) {
        ...
        if (settings.voiceResponse) {
            ...
            // Audio-routing settle: Android's hardware audio path takes
            // 100–300 ms to switch from speaker-output back to mic-input
            // mode after audio focus is released.  Without this gap the
            // SpeechRecognizer opens its session while the codec is still
            // in playback routing — causing ERROR_AUDIO or silent results
            // on the very next listen(), which is the primary reason "Chris"
            // was never heard in onboarding.  Also gives BargeInDetector's
            // AudioRecord.release() time to fully complete on Dispatchers.Default
            // before SpeechCapture tries to claim the same hardware mic.
            delay(250L)
        }
    ...
    private fun parseOwnerName(transcript: String): String? {
        val t = transcript.trim()
        // "My name is Chris" / "I'm Chris" / "It's Chris" / bare "Chris"
        val explicit = Regex(
            """(?:my name is|i(?:'?m| am)|it(?:'?s| is))\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\b""",
            RegexOption.IGNORE_CASE
        ).find(t)?.groupValues?.get(1)?.trim()
        if (explicit != null) return explicit.replaceFirstChar { it.uppercase() }
        // Bare single/double name token — no filler words
        val bare = Regex("""^([A-Z][a-z]{1,19}(?:\s+[A-Z][a-z]{1,19})?)$""", RegexOption.IGNORE_CASE)
            .find(t)?.groupValues?.get(1)?.trim()
        return bare?.split(" ")?.joinToString(" ") { it.replaceFirstChar { c -> c.uppercase() } }
    }
