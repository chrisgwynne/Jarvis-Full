...
class SpeechCapture(private val context: Context) {
    ...
    companion object {
        ...
        // 5 chars guards short names ("Chris"=5, "Anna"=4) from being cut
        // before the recogniser has fully assembled the result.  The old
        // value of 2 caused the VAD timer to fire after the first partial
        // syllable appeared, dropping single-word onboarding responses.
        private const val FAST_VAD_MIN_PARTIAL_CHARS = 5

        // Default (phone-mic) thresholds.  Overridden per-profile via setMicProfile().
        // Lowered from 2.0f → 1.2f so speech at normal conversational volume
        // (arm's-length phone, not shouting) clears the gate.  The previous
        // 2.0f threshold required noticeably louder-than-normal speech.
        private const val DEFAULT_VAD_RMS_THRESHOLD  = 1.2f
        // Extended from 350 ms → 600 ms: gives the cloud recogniser more time
        // to return partial results for short utterances before the VAD cuts it.
        private const val DEFAULT_VAD_SILENCE_MS     = 600L
    ...
    private suspend fun listenInternal(
        ...
        mainHandler.post {
            ...
            val intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply {
                ...
                // Do NOT prefer offline: the cloud model is significantly more
                // accurate for short utterances and proper nouns (e.g. names
                // in onboarding).  The on-device model requires noticeably
                // louder speech and degrades badly on single-word responses.
                // Offline is still used automatically when the device is truly
                // offline (set by the forceOffline path above) — but when
                // network is available we want the best recognition quality.
